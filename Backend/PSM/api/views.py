...
class SeedDataView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # 1. Crear Grupos de usuario
            admin_group, _ = Group.objects.get_or_create(name='admin')
            cliente_group, _ = Group.objects.get_or_create(name='cliente')
            mensajes.append("Grupos 'admin' y 'cliente' verificados.")
            
            # 2. Asignar rol admin al superusuario existente
            if Usuario.objects.filter(is_superuser=True).exists():
                admins = Usuario.objects.filter(is_superuser=True)
                for admin in admins:
                    admin.groups.add(admin_group)
                    mensajes.append(f"Rol 'admin' asignado correctamente a {admin.username}")
            
            # 3. Crear Categorías base si no existen
            categorias_data = [
                {"nombre": "Conciertos", "descripcion": "Eventos musicales en vivo"},
                {"nombre": "Deportes", "descripcion": "Actividades deportivas y torneos"},
                {"nombre": "Teatro", "descripcion": "Obras de teatro y artes escénicas"},
                {"nombre": "Tecnología", "descripcion": "Conferencias y talleres tech"},
                {"nombre": "Gastronomía", "descripcion": "Ferias de comida y degustaciones"}
            ]
            
            for cat_data in categorias_data:
                cat, created = CategEvento.objects.get_or_create(nombre=cat_data['nombre'], defaults=cat_data)
                if created: mensajes.append(f"Categoría '{cat.nombre}' creada.")
            
            # 4. Crear Ubicaciones base
            ubicaciones_data = [
                {"recinto": "Estadio Nacional", "direccion": "https://goo.gl/maps/example1", "telefono_contacto": "2222-2222"},
                {"recinto": "Teatro Melico Salazar", "direccion": "https://goo.gl/maps/example2", "telefono_contacto": "2222-3333"},
                {"recinto": "Centro de Convenciones", "direccion": "https://goo.gl/maps/example3", "telefono_contacto": "2222-4444"}
            ]
            
            for ubi_data in ubicaciones_data:
                ubi, created = Ubicacion.objects.get_or_create(recinto=ubi_data['recinto'], defaults=ubi_data)
                if created: mensajes.append(f"Ubicación '{ubi.recinto}' creada.")
            
            # 5. Crear Evento de prueba si la base de datos está vacía
            if Evento.objects.count() == 0:
                cat_concierto = CategEvento.objects.get(nombre="Conciertos")
                ubi_estadio = Ubicacion.objects.get(recinto="Estadio Nacional")
                
                Evento.objects.create(
                    nombre="Gran Concierto de Rock",
                    descripcion="El mejor evento de rock del año.",
                    categoria=cat_concierto,
                    ubicacion=ubi_estadio,
                    ...
                    dias_semana=["sábado"],
                    cupo_maximo=5000,
                    cupos_disponibles=5000,
                    estado='activo',
                    imagen_id="sample_rock_concert" # Placeholder
                )
                mensajes.append("Evento de prueba 'Gran Concierto de Rock' creado.")
                
            return Response({'success': True, 'log': mensajes})
        except Exception as e:
            import traceback
            traceback.print_exc()
